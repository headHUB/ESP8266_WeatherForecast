...
void httpd_handleRootGet() {
  ...
  if (g_pHttpd != NULL) {
    ...
    html += "<h1>WiFi & CityID & AppKey Settings</h1>";
    html += "<form method='post'>";
    html += "  <input type='text' name='ssid' placeholder='ssid'><br>";
    html += "  <input type='text' name='pass' placeholder='pass'><br>";
    html += "  <input type='text' name='cityid' placeholder='cityid'><br>";
    html += "  <input type='text' name='appid' placeholder='appid'><br>";
...
void httpd_handleRootPost() {
  ...
  if( g_pHttpd != NULL ){
    ...
    String cityid = g_pHttpd->arg("cityid");
    String appid = g_pHttpd->arg("appid");
    ...
    if( (ssid=="format") && (pass=="format") ){
      ...
      Weather::saveWeatherConfig(cityid, appid);
    }
    
    String html = HTML_HEAD;
    html += "<h1>WiFi/Weather Settings</h1>";
    html += ((ssid!="") ? ssid : "ssid isn't changed") + "<br>";
    html += ((pass!="") ? pass : "password isn't changed") + "<br>";
    html += ((cityid!="") ? cityid : "cityId isn't changed") + "<br>";
    html += ((appid!="") ? appid : "appId isn't changed") + "<br>";
    html += HTML_TAIL;
